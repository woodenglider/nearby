...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if ([self supportsSwipingForCellAtPoint:gestureStartPoint]){
		[self performSelector:@selector(highlightTouchedRow) withObject:nil afterDelay:0.01];	
	}
	else
	{
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if ([self supportsSwipingForCellAtPoint:gestureStartPoint]){
		...
		if ([touch.view isKindOfClass:[TISwipeableTableViewCellView class]] 
            && cell.isSelected 
			&& !cell.contentViewMoving 
